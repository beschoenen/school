...
const std::string Socket::clear = "\033[2J";
const std::string Socket::prompt = "machiavelli> ";
...
Socket::Socket(SOCKET sock, const struct sockaddr& address) : sock{sock}
{
    switch (address.sa_family)
    {
        ...
        case AF_INET6:
        {
            auto* in6_addr = reinterpret_cast<struct sockaddr_in6*>(&addr);
...
std::string Socket::get_dotted_ip() const
{
    ...
    const auto* addr_p = reinterpret_cast<const struct sockaddr*>(&addr);

    switch (addr_p->sa_family)
    {
        case AF_INET:
        {
            const auto* in_p = reinterpret_cast<const struct sockaddr_in*>(addr_p);
            throw_if_null(result = ::inet_ntop(AF_INET, (void*) &in_p->sin_addr, textbuf, INET6_ADDRSTRLEN));
            break;
        }
        case AF_INET6:
        {
            const auto* in6_p = reinterpret_cast<const struct sockaddr_in6*>(addr_p);
...
ClientSocket::ClientSocket(const std::string& host, int port)
{
    ...
    if (gai_error)
    {
        ...
        oss << "getaddrinfo error " << gai_error << ": " << gai_strerror(gai_error)
            << " (" << __FILE__ << ":" << __LINE__ << ")";
